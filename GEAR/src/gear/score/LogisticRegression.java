...
public class LogisticRegression
{
	...
	public void MLE()
	{
		...
		if (!(new LUDecompositionImpl(Matrix_XT_W_X).getSolver()
				.isNonSingular()))
		{
			Logger.printUserError("Covariate matrix is singular.");
			System.exit(1);
		}
		RealMatrix Inv_XT_W_X = new LUDecompositionImpl(Matrix_XT_W_X)
		...
		do
		{
			...
			// System.out.println("LogLikelihood_old " + LogLikelihood_old +
			// ", LogLikelihood_new " + LogLikelihood_new);
		} while (iter++ < max_iteration
				&& Math.abs(LogLikelihood_old - LogLikelihood_new) > tolerance);
		// System.out.println();
	}
	...
	public static void main(String[] args)
	{
		...
		/*
		 * String filename = "0.phe"; FileReader r = null; try { r = new
		 * FileReader(filename); } catch(IOException E) {
		 * E.printStackTrace(System.err); } ArrayList<ArrayList>
		 * missingGenotypeInformation = new ArrayList(); LineNumberReader l =
		 * new LineNumberReader(r); BufferedReader b = new BufferedReader(l);
		 * String line; try { if ((line = b.readLine()) == null) { } } catch
		 * (IOException E) { E.printStackTrace(System.err); } Vector<String> Phe
		 * = new Vector<String>(); Vector<String> Cov = new Vector<String>();
		 * try { while ((line = b.readLine()) != null) { String[] fields =
		 * line.split("\t"); Cov.add(fields[2]); Phe.add(fields[3]); } } catch
		 * (IOException E) { E.printStackTrace(System.err); }
		 * 
		 * double[] phe = new double[Phe.size()]; double[][] cov = new
		 * double[Cov.size()][1]; for(int i = 0; i < phe.length; i++) {
		 ...
		 * LogReg1.MLE(); double[] res = LogReg1.getResiduals1(); for(int i = 0;
		 * i < res.length; i++) { System.out.println(res[i]); }
		 */
	}
}
