...
public final class SimuFamilyCommandImpl extends CommandImpl
{
	...
	private void getDomEffect()
	{
		...
		{
			for (int i = 0; i < idx.length; i++) deffect[idx[i]] = famArgs.getPolyDomEffect();
		}
		else if (famArgs.isPolyDomEffect())
		{
			...
			if (famArgs.isPolyDomEffect())
			{
				if (famArgs.getHsq() > 0)
				{
					...
					if (t < 2)
					{
						Logger.printUserError("Impossible heritability: hsq_add = " + famArgs.getHsq() + " hsq_dom = " + famArgs.getHsqDom());
						System.exit(0);
					}
					else
					{
		...
		else if (famArgs.isPolyDomEffectFile())
		{
			...
			try
			{
				while ((line = reader.readLine()) != null)
				{
					if(c >= M)
					{
						Logger.printUserLog("Have already read " + M + " allelic effects.  Ignore the rest of the content in '" + famArgs.getFreqFile() + "'.");
						break;
					}
