...
public final class SimuFamilyCommand extends Command {
	...
	public void prepareOptions(Options options) {
		options.addOption(OptionBuilder.withDescription(OPT_NUM_FAMS_DESC).withLongOpt(OPT_NUM_FAMS_LONG).hasArg()
				.create(OPT_NUM_FAMS));
		options.addOption(OptionBuilder.withDescription(OPT_NUM_MARKERS_DESC).withLongOpt(OPT_NUM_MARKERS_LONG).hasArg()
				.create(OPT_NUM_MARKERS));
		options.addOption(OptionBuilder.withDescription(OPT_NULL_MARKER_LONG_DESC).withLongOpt(OPT_NULL_MARKER_LONG)
				.hasArg().create());

		options.addOption(OptionBuilder.withDescription(OPT_SEED_DESC).withLongOpt(OPT_SEED_LONG).hasArg().create());
		options.addOption(
				OptionBuilder.withDescription(OPT_MAKE_BED_DESC).withLongOpt(OPT_MAKE_BED_LONG).create(OPT_MAKE_BED));
		...
		options.addOption(
				OptionBuilder.withDescription(OPT_UNIF_MAF_LONG_DESC).withLongOpt(OPT_UNIF_MAF_LONG).create());
		options.addOption(OptionBuilder.withDescription(OPT_FREQ_FILE_LONG_DESC).withLongOpt(OPT_FREQ_FILE_LONG)
				.hasArg().create());

		options.addOption(OptionBuilder.withDescription(OPT_REC_DESC).withLongOpt(OPT_REC_LONG).hasArg().create());
		options.addOption(OptionBuilder.withDescription(OPT_REC_SEX_DESC).withLongOpt(OPT_REC_SEX_LONG).hasArgs(2)
				.create(OPT_REC_SEX));
		...
		// options.addOption(OptionBuilder.withDescription("Scale
		// IBD").create(OPT_SCALE));
		options.addOption(OptionBuilder.withDescription(OPT_EFFECT_FILE_LONG_DESC).withLongOpt(OPT_EFFECT_FILE_LONG)
				.hasArg().create());

		options.addOption(
				OptionBuilder.withDescription(OPT_DOM_EFFECT_DESC).hasArg().withLongOpt(OPT_DOM_EFFECT).create());
		options.addOption(OptionBuilder.withDescription(OPT_POLY_DOM_EFFECT_LONG_DESC)
				.withLongOpt(OPT_POLY_DOM_EFFECT_LONG).create());
		options.addOption(OptionBuilder.withDescription(OPT_POLY_DOM_EFFECT_SORT_LONG_DESC)
				.withLongOpt(OPT_POLY_DOM_EFFECT_SORT_LONG).create());
		options.addOption(OptionBuilder.withDescription(OPT_DOM_EFFECT_FILE_LONG_DESC)
				.withLongOpt(OPT_DOM_EFFECT_FILE_LONG).hasArg().create());
	...
	// private static final String OPT_QTL_DESC = "qtl parameters (locp, locm, effm,
	// effp, h2)";
