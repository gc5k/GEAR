...
public final class ProfileCommandImpl extends CommandImpl {
	...
	public void execute(CommandArguments cmdArgs) {
		...
		if (profCmdArgs.getExtractScoreFile() != null) {
			extractSCsnp = NewIt.newHashSet();
			BufferedReader reader = BufferedReader.openTextFile(profCmdArgs.getExtractScoreFile(),
					"Profile Extract score");
			String[] tokens = null;
			...
			Logger.printUserLog("Read " + extractSCsnp.size() + " SNPs in " + profCmdArgs.getExtractScoreFile());
		}

		if (profCmdArgs.getRemoveScoreFile() != null) {
			extractSCsnp = NewIt.newHashSet();
			BufferedReader reader = BufferedReader.openTextFile(profCmdArgs.getRemoveScoreFile(),
					"Profile Remove score");
			String[] tokens = null;
			...
			Logger.printUserLog("Read " + extractSCsnp.size() + " SNPs in " + profCmdArgs.getRemoveScoreFile());
			isKeepSC = false;
		}
	...
	private void readCoeffModelFile(String fileName, SNP[] snps, CoeffModel[] coeffModels) {
		...
		while ((tokens = reader.readTokens(2)) != null) {
			...
			if (modelName.equalsIgnoreCase("ADD")) {
				...
				reader.errorPreviousLine(
						"Unrecognized model '" + modelName + "'. Model name must be 'ADD', 'DOM' or 'REC'");
			}
	...
	private void printSNPFilterResult(ScoreFile scoreFile, QRange[] qRanges, FilteredSNPs filteredSNPs) {
		...
		Logger.printUserLog("Number of ambiguous loci (A/T or C/G) "
				+ (profCmdArgs.getIsKeepATGC() ? "detected: " : "removed: ") + filteredSNPs.getNumAmbiguousLoci());

		// Allele Matching Schemes
		Logger.printUserLog("Number of Scheme I predictors: predictor alleles were A1: "
				+ filteredSNPs.getMatchNum(AlleleMatchScheme.MATCH_ALLELE1));
		Logger.printUserLog("Number of Scheme II predictors: predictor alleles were A2: "
				+ filteredSNPs.getMatchNum(AlleleMatchScheme.MATCH_ALLELE2));
		if (profCmdArgs.getIsAutoFlip()) {
			Logger.printUserLog("Number of Scheme III predictors: predictor alleles were flipped A1: "
					+ filteredSNPs.getMatchNum(AlleleMatchScheme.MATCH_ALLELE1_FLIPPED));
			Logger.printUserLog("Number of Scheme IV predictors: predictor alleles were flipped A2: "
					+ filteredSNPs.getMatchNum(AlleleMatchScheme.MATCH_ALLELE2_FLIPPED));
		}
		Logger.printUserLog("Number of score alleles matching none in the data file: "
				+ filteredSNPs.getMatchNum(AlleleMatchScheme.MATCH_NONE));
		...
		if (profCmdArgs.getQScoreFile() != null) {
			...
			for (int i = 0; i < filteredSNPs.getNumLocusGroups(); i++) {
				Logger.printUserLog("Number of loci within the range: " + qRanges[i].getLowerBound() + ", "
						+ qRanges[i].getUpperBound() + " is " + filteredSNPs.getNumInLocusGroup(i));
			}
		}

		for (int traitIdx = 0; traitIdx < scoreFile.getNumberOfTraits(); ++traitIdx) {
			Logger.printUserLog("Number of loci having no score for trait " + scoreFile.getTrait(traitIdx) + ": "
					+ filteredSNPs.getNumLociNoScore(traitIdx));
		}
	}
			...
			if (!ConstValues.isNA(tokens[1])) {
				try {
					if (qScores.containsKey(tokens[0])) {
						Logger.printUserLog("Warning: Marker '" + tokens[0] + "' duplicated in '"
								+ profCmdArgs.getQScoreFile() + "'" + ", the first instance used, others skipped.");
						continue;
					}
			...
			try {
				...
				for (QRange qRange : qRanges) {
					if (qRange.getName().equals(tokens[0])) {
						Logger.printUserError("QRange '" + qRange.getName()
								+ "' appears more than once in the qrange file '" + profCmdArgs.getQRangeFile() + "'.");
						System.exit(1);
					}
				}
	...
	private Data initData() {
		...
		if (pp == null) {
			...
			return MachData.create(profCmdArgs.getMachDosageFile(), profCmdArgs.getMachInfoFile(),
				...
				if (scaleMaf.size() > 0) {
					for (int i = 0; i < pGM.getSNPList().size(); i++) {
						...
						if (scaleMaf.containsKey(snp.getName())) {
							...
							switch (FilteredSNPs.getMatchScheme(sm.getA1(), snp.getFirstAllele(), snp.getSecAllele(),
									profCmdArgs.getIsAutoFlip())) {
							case MATCH_ALLELE1:
							case MATCH_ALLELE1_FLIPPED:
								freq[i][0] = sm.getMaf();
