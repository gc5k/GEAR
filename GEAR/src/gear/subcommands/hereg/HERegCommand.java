...
public class HERegCommand extends Command {
	...
	public void prepareOptions(Options options) {
		...
		options.addOption(
				OptionBuilder.withDescription(OPT_GRM_LIST_DESC).withLongOpt(OPT_GRM_LIST_LONG).hasArg().create());
		...
		options.addOption(OptionBuilder.withDescription(OPT_JACKKNIFE_DESC).withLongOpt(OPT_JACKKNIFE_LONG)
				.create(OPT_JACKKNIFE));
		options.addOption(
				OptionBuilder.withDescription(OPT_GRM_CUTOFF_DESC).withLongOpt(OPT_GRM_CUTOFF_LONG).hasArg().create());
	}
		...
		if (grmList != null) {
			...
			if (numFiles == 0) {
				throw new CommandArgumentException("No GRM is provided. One of --" + OPT_GRM_BIN_LONG + ", "
						+ OPT_GRM_TEXT_LONG + " or --" + OPT_GRM_GZ_LONG + " must be set.");
			}
			if (numFiles > 1) {
				throw new CommandArgumentException("At most one of --" + OPT_GRM_BIN_LONG + ", --" + OPT_GRM_TEXT_LONG
						+ " and --" + OPT_GRM_GZ_LONG + " can be set.");
			}
		}
	}
