...
//				HESingleJackknife();
	...
	private void HEMultple() {
		...
		for (int i = 0; i < Y.length; i++) {
			for (int j = 0; j < i; j++) {
				...
				if (heArgs.isGRMcut() && A3[0][i][j] > heArgs.getGRMcutoff())
					continue;
	...
	private void HESingle() {
		...
		jxySum = new double[Y.length];
		jxSum = new double[Y.length];
		jySum = new double[Y.length];
		jxSqSum = new double[Y.length];
		...
		for (int i = 0; i < Y.length; i++) {
			for (int j = 0; j < i; j++) {
				...
				ySum += yy[cnt];
				xSum += xx[cnt][0];
				xySum += yy[cnt]*xx[cnt][0];
				xSqSum += xx[cnt][0]*xx[cnt][0];
				...
				jxySum[i] += yy[cnt]*xx[cnt][0];
				jxySum[j] += yy[cnt]*xx[cnt][0];
				
				jxSqSum[i] += xx[cnt][0]*xx[cnt][0];
				jxSqSum[j] += xx[cnt][0]*xx[cnt][0];
	...
	private void HESingleJackknifeSmart() {

		double[] jb = new double[Y.length];
		double jbSum = 0;
		for (int i = 0; i < Y.length; i++) {
			
			double xySum_ = xySum - jxySum[i];
			double xSum_ = xSum - jxSum[i];
			double ySum_ = ySum - jySum[i];
			double xSqSum_ = xSqSum - jxSqSum[i];
			double cnt_ = cnt - Y.length;
			double b_= (xySum_ - xSum_ * ySum_/cnt_)/(xSqSum_ - xSum_ * xSum_/cnt_);
			jb[i] = Y.length * beta[1] - (Y.length-1) * b_;
			jbSum += jb[i];
		}
		double jb_ = jbSum / Y.length;
		double jbS = 0;
		for (int i = 0; i < Y.length; i++) {
			jbS += (jb_-jb[i])*(jb_-jb[i]);
		}
		jbS = Math.sqrt(jbS/(Y.length*(Y.length-1)));
		System.out.println(jb_ + " " + jbS);
		
		Logger.printUserLog("-------------------------------------------------------------------");
		Logger.printUserLog("Jackknife results: ");
//		Logger.printUserLog("JK_Mean\t" + jb_ + "\t" + jbS);
		Logger.printUserLog("JK_Beta1\t" + jb_ + "\t" + jbS);
		Logger.printUserLog("-------------------------------------------------------------------");
	}
	...
	private double[] jxySum = null;
	private double[] jxSum = null;
	private double[] jySum = null;
	private double[] jxSqSum = null;
