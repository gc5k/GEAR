...
//		missList = PopStat.punchMissingGenoMT(pGM, cmdArgs.isThreadNum()? cmdArgs.getThreadNum():1);
		missG = PopStat.punchMissingGenoMT2(pGM, cmdArgs.isThreadNum()? cmdArgs.getThreadNum():1);
			...
			Thread thread = new Thread() {
				public void run() {
					...
					do {
						...
						List<Integer> mL1 = NewIt.newArrayList();
						if (missG[sampleIndex1] != null) {
							for(int j = 0; j < missG[sampleIndex1].length; j++) {
								mL1.add(missG[sampleIndex1][j]);
								ws1 -= weight[missG[sampleIndex1][j]]*weight[missG[sampleIndex1][j]];	
							}
						}
						...
						do {
							...
							List<Integer> mL2 = NewIt.newArrayList();
							if (missG[sampleIndex2] != null) {
								for(int j = 0; j < missG[sampleIndex2].length; j++) {
									mL2.add(missG[sampleIndex2][j]);
									ws2 -= weight[missG[sampleIndex2][j]]*weight[missG[sampleIndex2][j]];	
								}
							}
	...
	private void makeGAMemLow2() {
		Logger.printUserLog("");
		Logger.printUserLog("Making additive genetic relatedness matrix using low memory...");
		long startNanoTime = System.nanoTime();
		int numSamples = pGM.getNumIndivdial();
				...
				public void run() {
					...
					do {
						...
						do {
							...
							int mLoci = pGM.getNumMarker();//gMat[sampleIndex1].length;
							int cnt = 0;
							if ( mL1.size() <= mL2.size() ) {
								for (int k = 0; k < mL1.size(); k++) {
									if (Collections.binarySearch(mL2, mL1.get(k)) >=0) {
							...
							float s = 0;
							for (int l = 0; l < pGM.getNumMarker(); l++) {
								float f = allelefreq[l][1];
								if (Float.isNaN(allelefreq[l][0]) || allelefreq[l][0] == 0 || allelefreq[l][1] == 0 || allelevar[l] == 0) continue;

								float de = 0;
								if (wgrmArgs.isInbred()) {
									de = (float) Math.sqrt(4*allelefreq[l][1]*allelefreq[l][0]);
								} else if (wgrmArgs.isAdjVar()) {
									de = (float) Math.sqrt(allelevar[l]);
								} else {
									de = (float) Math.sqrt(2*allelefreq[l][1]*allelefreq[l][0]);
								}

								int g1 = pGM.getAdditiveScore(sampleIndex1, l);
								float gs1 = 0;
								int g2 = pGM.getAdditiveScore(sampleIndex2, l);
								float gs2 = 0;
								if (g1 != Person.MissingGenotypeCode && g2 != Person.MissingGenotypeCode) {
									gs1 = (float) ((g1*1.0 - 2*f)/de * weight[l]);
									gs2 = (float) ((g2*1.0 - 2*f)/de * weight[l]);									
									s += gs1 * gs2;
								}
							}
							GA[startGrmIndex + taskProgress] = s/ws2;
				...
				public void run() {
					...
					do {
						...
						if (missG[sampleIndex1] != null) {
							for(int j = 0; j < missG[sampleIndex1].length; j++) {
								mL1.add(missG[sampleIndex1][j]);
								ws1 -= weight[missG[sampleIndex1][j]]*weight[missG[sampleIndex1][j]]*weight[missG[sampleIndex1][j]]*weight[missG[sampleIndex1][j]];	
							}
						}
						...
						do {
							...
							if (missG[sampleIndex2] != null) {
								for(int j = 0; j < missG[sampleIndex2].length; j++) {
									mL2.add(missG[sampleIndex2][j]);
									ws2 -= weight[missG[sampleIndex2][j]]*weight[missG[sampleIndex2][j]]*weight[missG[sampleIndex2][j]]*weight[missG[sampleIndex2][j]];
								}
							}

							int mLoci = pGM.getNumMarker();
							int cnt = 0;
							if ( mL1.size() <= mL2.size() ) {
								for (int k = 0; k < mL1.size(); k++) {
									if (Collections.binarySearch(mL2, mL1.get(k)) >=0) {
										ws2 += weight[mL1.get(k)]*weight[mL1.get(k)]*weight[mL1.get(k)]*weight[mL1.get(k)];
										cnt++;
									}
								}
							} else {
								for (int k = 0; k < mL2.size(); k++) {
									if (Collections.binarySearch(mL1, mL2.get(k)) >=0) {
										ws2 += weight[mL2.get(k)]*weight[mL2.get(k)]*weight[mL2.get(k)]*weight[mL2.get(k)];
										cnt++;
									}
								}
							}
							...
							for (int l = 0; l < pGM.getNumMarker(); l++) {
								...
								if (g1 != Person.MissingGenotypeCode) {
									if (g1 == 0) {
										gs1 = (0 - 2*f*f) / de * weight[l] * weight[l];
									} else if (g1 == 1) {
										gs1 = (2*f - 2*f*f) / de * weight[l] * weight[l];
									} else {
										gs1 = ((4*f - 2) -(2*f*f)) / de * weight[l] * weight[l];
									}
								}
								...
								if (g2 != Person.MissingGenotypeCode) {
									if (g2 == 0) {
										gs2 = (0 - 2*f*f) / de * weight[l] * weight[l];
									} else if (g2 == 1) {
										gs2 = (2*f - 2*f*f) / de * weight[l] * weight[l];
									} else {
										gs2 = ((4*f - 2) -(2*f*f)) / de * weight[l] * weight[l];
									}
								}
								if (g1 != Person.MissingGenotypeCode && g2 != Person.MissingGenotypeCode) {
									s += gs1 * gs2;
