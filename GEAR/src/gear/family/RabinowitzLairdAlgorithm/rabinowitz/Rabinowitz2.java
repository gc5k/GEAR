package gear.family.RabinowitzLairdAlgorithm.rabinowitz;
...
public class Rabinowitz2 extends AbstractGenoDistribution
{
	...
	public Rabinowitz2(TreeMap<String, Integer> children,
	...
	public String[] getNontransmitted()
	{
		...
		if (childrenGenoMap.size() == 1)
		{
			if (numHomozygous(childrenGenoMap) == 1
					|| childrenGenoMap.containsKey(parentgeno1))
			{// situation
				// 1
			...
			{// situation 4-1
				...
				String DeducedGeno1 = ExtractUniqueAllele2Genotype(
						childrenGenoMap.firstKey(), parentgeno1);
		...
		} else if (childrenGenoMap.size() == 2)
		{
			...
			{// situation
				...
				if (childrenGenoMap.containsKey(parentgeno1))
				{// 5-1
					// System.err.println("Rabinowitz Table2 s5-1");

					String DeducedGeno1 = ExtractUniqueAllele2Genotype(
							childrenGenoMap.firstKey(),
				...
				} else
				{
					if (numChildrenAllele() == 3)
					{
						...
						if (CASet.contains(parentgeno1.substring(0, 1))
								&& CASet.contains(parentgeno1.substring(1, 2)))
						{// 4-2
						...
						{// 7-2
							...
							String Deducedgeno1 = ExtractUniqueAllele2Genotype(
									childrenGenoMap.firstKey(), parentgeno1);
							String Deducedgeno2 = ExtractUniqueAllele2Genotype(
									childrenGenoMap.lastKey(), parentgeno1);
							genopool[0] = childrenGenoMap.firstKey();
							genopool[1] = childrenGenoMap.lastKey();
							genopool[2] = Deducedgeno1;
				...
				{// 6-3, 6-4
					...
					for (String g : childrenGenoMap.keySet())
					{
						...
						if (isHeterozygous(genopool[index])
								&& genopool[index].compareTo(parentgeno1) != 0)
						{
	...
	String CompatibleGenotype()
	{
		...
		for (String g : childrenGenoMap.keySet())
		{
			for (int i = 0; i < 2; i++)
			{
				...
				{
					alleleMap.put(new String(g.substring(i + 0, i + 1)),
							new Integer(1));
				}
			}
		}
