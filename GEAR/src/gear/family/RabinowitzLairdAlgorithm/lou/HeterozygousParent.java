package gear.family.RabinowitzLairdAlgorithm.lou;
...
public class HeterozygousParent extends AbstractGenoDistribution
{
	 ...
	 */
	public HeterozygousParent(TreeMap<String, Integer> children,
	...
	public String[] getNontransmitted(final String transmitted)
	{
		...
		} else if (childrenGenoMap.size() == 2)
		{
			...
			{// situation 4
				...
				nontran = new String(
						(tran.compareTo(childrenGenoMap.firstKey()) != 0) ? childrenGenoMap
								.firstKey() : childrenGenoMap.lastKey());

				if (!isHeterozygous(transmitted))
				{// 4-1
					nontran = new String((transmitted.compareTo(childrenGenoMap
							.firstKey()) != 0) ? childrenGenoMap.firstKey()
							: childrenGenoMap.lastKey());
				} else
				{// 4-2
					double vk1 = (childrenGenoMap.get(childrenGenoMap
							.firstKey())).doubleValue();
					double vk2 = (childrenGenoMap
							.get(childrenGenoMap.lastKey())).doubleValue();
			...
			} else if (numChildrenAllele() == 3)
			{
				...
				{// situation 14
					...
					if (tran.compareTo(parentgeno1) == 0)
					{
						...
						if (rd < 0.5)
						{
							nontran = ExtractUniqueAllele2Genotype(
