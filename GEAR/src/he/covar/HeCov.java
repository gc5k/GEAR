...
public class HeCov
{
	...
	private void generate_Res()
	{
		...
		if (qcov_file != null)
		{
			...
			try
			{
				while ((line = reader.readLine()) != null)
				{
					...
					{
						if (qcov_idx == null)
						{// use all the covariates if no covariates are selected
					...
					if (ID2Idx.containsKey(sb.toString()))
					{
						...
						cov_flag[idx] = f & flag[idx]; // it should exist
														// phenotype
						if (f)
						{
		...
		RealMatrix XtX_Inv = (new LUDecompositionImpl(XtX)).getSolver()
				.getInverse();
