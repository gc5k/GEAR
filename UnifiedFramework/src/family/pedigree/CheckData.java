...
public class CheckData {
    ...
    private MarkerResult checkMarker(int loc) throws MDRPedFileException,
        ...
        Enumeration famList = pedFile.getFamStrList();
        while (famList.hasMoreElements()) {
            FamilyStruct currentFamily = pedFile.getFamilyStruct((String) famList.nextElement());
            Enumeration indList = currentFamily.getPersonList();
            // loop through each individual in the current Family
            while (indList.hasMoreElements()) {
                currentInd = currentFamily.getPerson((String) indList.nextElement());
                ...
                if (allele1 > 0 && allele2 > 0) {
                    ...
                    if (currentFamily.containsPerson(currentInd.getMomID()) && currentFamily.containsPerson(currentInd.getDadID())) {
                        // do mendel check
                        int momAllele1 = (currentFamily.getPerson(currentInd.getMomID())).getAllele(loc, 0);
                        int momAllele2 = (currentFamily.getPerson(currentInd.getMomID())).getAllele(loc, 1);
                        int dadAllele1 = (currentFamily.getPerson(currentInd.getDadID())).getAllele(loc, 0);
                        int dadAllele2 = (currentFamily.getPerson(currentInd.getDadID())).getAllele(loc, 1);
                                ...
                                if (!(momAllele1 == 0 || momAllele2 == 0 || dadAllele1 == 0)) {
                                    ...
                                    if (allele1 != momAllele1 && allele1 != momAllele2) {
                                        ...
                                        MendelError mend = new MendelError(
                                                ...
                                                currentInd.getPersonID());
                                        mendels.add(mend);
                                        currentInd.zeroOutMarker(loc);
                                        currentFamily.getPerson(
            ...
            indList = currentFamily.getPersonList();
            // loop through each individual in the current Family
            while (indList.hasMoreElements()) {
                currentInd = currentFamily.getPerson((String) indList.nextElement());
                ...
                if (allele1 > 0 && allele2 > 0) {
                    ...
                    if (!currentFamily.hasAncestor(currentInd.getPersonID())) {
        ...
        // int famTrio =0;
        int famTrio = getNumOfFamTrio(pedFile.getFamStrList(), founderGenoCount,
                kidgeno);
