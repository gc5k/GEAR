...
public class RunPedSimulation {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < replication; i++) {
			...
			DataFile mdrData = new DataFile(GD.getMarkerName(), GD.getWorkingGenoTable(), GD.getWorkingStatusTable(),
					GD.getTraitName(), GD.getWorkingScoreTable(), gmdrPr.getScoreIndex());
			Subdivision sd = new Subdivision(gmdrPr.getInterval(), gmdrPr.getSeed() + i, mdrData);
			sd.RandomPartition();
			...
			as = new LinearMergeSearch(mdrData, sd, cg, gmdrPr.getScoreIndex().length, mdrData.getOffset(),
					gmdrPr.isMooreMDR());
			for (int j = gmdrPr.getInterctionFrom(); j <= gmdrPr.getInteractionEnd(); j++) {
				...
				for (int k = 0; k < (i > 0 ? 0 : pr.replication); k++) {
					...
					GD.CreateTableII(isRabinowitzProc);
					GD.CreateWorkingTable(isRabinowitzProc);
					DataFile mdrData1 = new DataFile(GD.getMarkerName(), GD.getWorkingGenoTable(),
							GD.getWorkingStatusTable(), GD.getTraitName(), GD.getWorkingScoreTable(),
