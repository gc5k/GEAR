...
public class GMDRData {
    ...
    // The nontransmitted genotypes followed the corresponding transmitted genotype
    ...
    private ArrayList FullCovariateTable;// phenotypes of informative individuals
    ...
    private ArrayList<PersonIndex> PersonTable;// The indexing file records the order of individuals in TransmittedTable
    // excluded parents
    private ArrayList<PersonIndex> FullPersonTable;// The indexing file records the order of all individuals
    ...
    private boolean PedFileType; // false for case-control design; true for family based design
    ...
    /**
     * PersonIndex class is developed to construct a unique key for each individual.
     * 
     * @author Guo-Bo Chen
     */
    public static class PersonIndex {
        ...
        /**
         * Constructs an individual index. Key = family_id+"->"individual_in within a family.
         * 
         * @param fid
     ...
     * 
     * @param Adjust
     *            a boolean variable. If it equals false, don't adjust the model with predictor.
     * @throws CalEngineException
     */
    public void buildScore2(int[] PIndex, int[] CIndex, boolean Adjust,
            int method, boolean includeFounder) throws CalEngineException {// method 0 for regression, 1 for logistic
        ...
        for (int i = 0; i < FullPersonTable.size(); i++) {
            ...
            if (PheIndex[0] == -1) {// using affecting status as phenotype
                if (((Integer) FullStatusTable.get(i)).intValue() == 0) {// ignor missing value
                    flag = false;
                    continue;
                ...
                if (((String) tempc.get(PheIndex[0])).compareTo(PublicData.MissingValue) == 0) {
                    flag = false;
                    continue;
                } else {
            ...
            if (Adjust) {
                for (int j = 0; j < CovIndex.length; j++) {
                    if (((String) tempc.get(CovIndex[j])).compareTo(PublicData.MissingValue) == 0) {
                        flag = false;
                        break;
                    }
    ...
    public void buildScore(int[] PIndex, int[] CIndex, boolean Adjust,
            int method) throws CalEngineException {// method 0 for regression, 1 for logistic
        ScoreHashTable.clear();
        PersonIndex PI;
        String key;
        scoreProc = method;
    ...
    public void CalculateScore(int[] CovIndex, int PhenoIndex,
            boolean adjustment, int method) throws CalEngineException // adjust=false ,no cov
    ...
    public void CalculateScore2(int[] CovIndex, int PhenoIndex, boolean adjust,
            int method) throws CalEngineException// only sibs are used in score calculation
    // adjust=false ,no cov
    // adjust=ture, with cov
    {
        double[][] status;
    ...
    /**
     * Clear all tables, including TDTGenoTable, CovariateTable, FulCovariateTable, StatusTable, TDTScoreTable,
     * TransmittedTable, PersonTable, FullPersonTable, NontransmittedTable.
     */
        ...
        for (int i = 0; i < FID.length; i++) {
            ...
            while (sibList.hasMoreElements()) {
                ...
                try {
                    ...
                    System.err.println("Can't find the individual " + pid + " genotype in family " + fid);
                } catch (GMDRPhenoFileException e) {
                    System.err.println("Can't find the individual " + pid + " phenotype in family " + fid);
                }
            }
        }
    }
    ...
    /**
     * When score has alread been built outside PedGMDR, it can be imported directly
     * 
     * @param score_index
    ...
    private boolean IsUnMatchedIndividual(String fid, String pid) {
        for (int i = 0; i < unMatched.size(); i++) {
            ...
            if (((String) tmp.get(0)).equals(fid) && ((String) tmp.get(1)).equals(pid)) {
                return true; // individual #iid in family #fid does not have phenotype
    ...
    /**
     * Check whether the individuals in the pedigree file match the one in the phenotype file.
     * 
     * @return
     */
                ...
                try {
                    ...
                    while (subList.hasMoreElements()) {
                        ...
                        try {
                            ...
                            System.err.println("no such individual with ID " + iid + " in family " + fid + " in the phenotype file.");
                        }
                    }
                    ...
                    System.err.println("no such individual with ID " + iid + " in family " + fid + "in the pedigree file.");
                }
            }
        }
        return true;
     ...
     * 
     * @param TDTped
     *            The file for printing out transmitted-nontransmitted genotypes.
     * @param TDTphe
    ...
    public void realPrintGMDR(String TDTped, String TDTphe, String TDTpi,
        ...
        for (int i = 0; i < PersonTable.size(); i++) {
            PI = PersonTable.get(i);
            if ((!((Boolean) faminformative.get(PI.getFamilyID())).booleanValue()) || (!ScoreHashTable.containsKey(PI.getKey())))// not informative or no score
            {
                countmiss++;
        ...
        for (int i = 0; i < TransmittedTable.size(); i++) {
            ...
            if ((((Integer) StatusTable.get(i)).intValue() - 1) == 1 && datatype == 1) {
                continue;
            }
        ...
        try {
            ...
            System.err.println("Could not find the phenotype at index " + PheIndex[0]);
        }
     ...
     * @throws CalEngineException
     */
    public void RabinowitzPrintGMDR(String TDTped, String TDTphe, boolean isPermutation) throws IOException, CalEngineException {
        ...
        for (int i = 0; i < PersonTable.size(); i++) {
            ...
            if ((!((Boolean) faminformative.get(PI.getFamilyID())).booleanValue()) 
                    || (!ScoreHashTable.containsKey(PI.getKey()))) { // not informative or no score
                countmiss++;
                Discard.add(PI.getKey());
                continue;
            }
        ...
        if( !isPermutation ) {
            ...
            for (int i = 0; i < PrintOutPersonTable.size(); i++) {
                String pi = PrintOutPersonTable.get(i);
                double s = ((Double) ScoreHashTable.get(pi)).doubleValue();
                TDTpheout.println(s);//print the scores for transtmitted phenotype only;
            }
            TDTpheout.close();
        }
    }
    ...
    private void TableConversion() {
        ...
        FullCovariates = new double[FullCovariateTable.size()][PhenoData.getNumTraits()];
        ArrayList cov;
        String co;
